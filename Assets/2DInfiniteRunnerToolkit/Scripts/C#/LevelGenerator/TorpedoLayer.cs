...
public class TorpedoLayer : MovingLayer
{
    ...
    {
		startAt = Camera.main.ScreenToWorldPoint(new Vector3(Screen.width * 1.2f, 0, 0)).x;
    ...
    public override void SpawnElement(bool inMiddle)
    {
        ...
        for (int i = 0; i < toSpawn; i++)
        {
			...
			float yPos = player.position.y + 0.75f;//Random.Range(minPosY, maxPosY);
			SpawnTorpedo(yPos);
	...
	public IEnumerator SpawnRateChanger ()
	{
		int i = Random.Range(0,100);
		if (i < 25)
		{
			spawningRate = Random.Range(0.75f,1.5f);
//it will wait long enough for a spawn * random amounst more of spawns (least amount to spawn, most amount)
			yield return new WaitForSeconds(spawningRate * Random.Range(3,6));
			StartCoroutine(SpawnRateChanger());
		}
		else
		{
			spawningRate = Random.Range(20,35);
