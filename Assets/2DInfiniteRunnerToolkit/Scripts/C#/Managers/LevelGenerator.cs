...
public class LevelGenerator : MonoBehaviour
{
	...
	public GameObject[] obstacleLayers;							//Holds the obstacles layers
	...
	public PowerupLayer waterDropLayer;							//A link to the waterdrop layer
	...
	public static float backChangePos;
	public static bool dead;
	public GameObject background;	//put ground backgrounds here
	...
	void Update ()
	{
		//turn on and off 1st section
		if (distance >= 34 && distance < 120)
		{
			if(!ol0)
			{
				obstacleLayers[0].SetActive(true);

				if(!PowerupManager.bassBoost)
				{
					movingLayers[2].StartObstacles();
					movingLayers[3].StartObstacles();
					movingLayers[4].StartObstacles();
					movingLayers[5].StartObstacles();
					movingLayers[6].StartObstacles();
				}
				ol0 = true;
			}
		...
		{
			movingLayers[6].StopAllCoroutines();
			if (movingLayers[6].activeElements.Count == 0)
				obstacleLayers[0].SetActive(false);
		}
		//turn on and off 2nd section
		if (distance >= 120 && distance < 250)
		{

			if(!ol1)
			{
				obstacleLayers[1].SetActive(true);

				if(!PowerupManager.bassBoost)
				{
					movingLayers[7].StartObstacles();
				}
				ol1 = true;
			}
		}
		...
		{
			movingLayers[7].StopAllCoroutines();
			if (movingLayers[7].activeElements.Count == 0)
				obstacleLayers[1].SetActive(false);

		}
		//turn on and off 3rd section
		if (distance >= 250 && distance < 350)
		{
			if(!ol2)
			{
				obstacleLayers[2].SetActive(true);

				if(!PowerupManager.bassBoost)
				{
					movingLayers[8].StartObstacles();
				}
				ol2 = true;
			}
		}
		...
		{
			movingLayers[8].StopAllCoroutines();
			if (movingLayers[8].activeElements.Count == 0)
				obstacleLayers[2].SetActive(false);
		}
		//turn on and off 4th section
		if (distance >= 350)
		{
			if(!ol3)
			{
				obstacleLayers[3].SetActive(true);

				if(!PowerupManager.bassBoost)
				{
					movingLayers[9].StartObstacles();
				}
				ol3 = true;
			}
		}
		else if (distance < 350 && obstacleLayers[3].activeSelf == true)
		{
			movingLayers[9].StopAllCoroutines();
			if (movingLayers[9].activeElements.Count == 0)
				obstacleLayers[3].SetActive(false);
		}
	}
    ...
    void FixedUpdate()
    {
        ...
        if (!paused)
        {
			...
			if (canModifySpeed && speedMultiplier < maxSpeedMultiplier)
            {
				...
				waterDropLayer.UpdateSpeedMultiplier(speedMultiplier);
                torpedoLayer.UpdateSpeedMultiplier(speedMultiplier);
            }
			else if (canModifySpeed && speedMultiplier >= maxSpeedMultiplier)
			{
		...
		if(distance >= 4000 && !firstMove)
		{
			StartCoroutine(ChangeBackground(backChangePos, 4.75f));
			firstMove = true;
		}
		if(distance >= 8000 && !secondMove)
		{
			StartCoroutine(ChangeBackground(backChangePos, 4.75f));
			secondMove = true;
		}
    ...
    IEnumerator ChangeBackground(float posX, float time)
    {
		...
		Vector3 endPos = new Vector3(background.transform.position.x - posX, background.transform.position.y, 0);
        ...
        while (i < 1.0)
        {
            ...
            if (!paused)
            {
				...
				background.transform.position = Vector3.Lerp(startPos, endPos, i);
            }
    ...
    //Changes the speed multiplier to newValue in time
   	public IEnumerator ChangeScrollingMultiplier(float newValue, float time, bool enableIncrease)
    {
        //Declare variables, get the starting position, and move the object
        float i = 0.0f;
	...
	public void ContinueGeneration()
    {
		...
		waterDropLayer.EnableSpawning(true);

		canModifySpeed = true;

		StartCoroutine(ChangeScrollingMultiplier(lastSpeedMultiplier, 3, true));
	}
	...
	public void StopGeneration(float time)
    {
		...
		waterDropLayer.EnableSpawning(false);
    ...
    public void SetPauseState(bool state)
    {
		...
		waterDropLayer.SetPauseState(state);
	...
	{
		lastSpeedMultiplier = speedMultiplier;
		StartCoroutine(ChangeScrollingMultiplier(1, 0, false));

		StartCoroutine(ChangeScrollingMultiplier(lastSpeedMultiplier, 3, false));
